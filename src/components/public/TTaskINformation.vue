...
export default {
  ...
  computed: {
    allCarTask: function() {
      return Object.values(this.$store.state.allMission);
    },

    carIdData: function() {
      return Object.keys(this.$store.state.allCarIdMission);
    },

    waitTask: function() {
      return Object.values(this.$store.state.waitMission);
    },

    finishTask: function() {
      return Object.values(this.$store.state.finishMission);
    },

    errorTask: function() {
      return Object.values(this.$store.state.errorMission);
    },
  ...
  methods: {
    inquireDeviceId(val) {
      let inquireIdTask = [];
      for (let item of this.allCarTask) {
        if (val === item.carId) {
