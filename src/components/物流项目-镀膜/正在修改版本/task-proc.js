...
const { taskOld } = require('../service/mission-center/task-manager');
...
const parseNodes = async(roadNode) => {
    ...
    for (const val of roadNode) {
        ...
        copyNode.todo.action = val.todo.action;
        copyNode.todo.target = val.todo.target;
...
// store to mysql
const storeTodb = async(mission) => {
    const logger1 = logger.scope('storeTodb');

    // 循环repeat存入数据库
    for (let i = 0; i < mission.repeat; i++) {
        await taskOld.insert(mission[i]);      
    }
