    ...
    for (let i = 0; i < roadNodesLength; i++) {
        ...
        if (cn !== true) {
            ...
            const cta = await checkField([
                ...
                [r.todo, 'direction', 'number'],
                [r.todo, 'endLevel', 'number']
            ]);
            if (cta !== true) {
                cta.forEach(element => {
        ...
        // goods
        const cg = await checkField([[r, 'goods', 'object']]);
        if (cg !== true) errors.push(str + cg);
        else {
            // type, weight
            const cgt = await checkField([
                [r.goods, 'type', 'number'],
                [r.goods, 'weight', 'number']]);
            if (cgt !== true) {
                cgt.forEach(element => {
                    errors.push(str + "'goods'中的" + element);
                });
            }
        }
    }
...
async function checkTask(mission) {
    const cc = await checkField([
        [mission, 'taskId', 'string'],
        [mission, 'preTaskId', 'string'],
        [mission, 'taskType', 'string'],
        [mission, 'boxId', 'string'],
