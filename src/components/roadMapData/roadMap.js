...
async function parseRoadNetData(roadMap) {
    ...
    for (let i = 1; i < laneLengths; i++) {
        const node = await parseNode(roadMap[i-1].lane);
        nodes.push(node[0]);
        nodes.push(node[1]);
        const line = await parseLine(roadMap[i-1]);
        lines.push(line);
        if (roadMap[i].map_name !== roadMap[i-1].map_name) {
           const ObjectA = { area: roadMap[i-1].map_name, nodes: nodes, lines: lines };
           roadData.push(ObjectA);
           nodes.splice(0, nodes.length);
           lines.splice(0, lines.length);
        }
    }
    roadData.push({ area: roadMap[laneLengths-1].map_name, nodes: nodes, lines: lines});
