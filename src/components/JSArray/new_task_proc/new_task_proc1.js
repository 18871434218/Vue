...
const { initClient } = require('../service/grpc/clientProtos/initClient.js');
...
const queryCarIpAndcurrentNode = async(Mission) => {
    ...
    const currentweight = await carManager.queryCurrentWeight(carId);
    return {
...
const missionFinish = async(ctx, next) => {
    try {
        ...
        if (currentCheckMission !== null) {
            ...
            const currentweight = queryData.currentweight;
