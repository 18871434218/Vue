...
const allTaskGeneration = async(ctx, next) => {
    ...
    if (num === 0) {
        ...
        for (const mission of allTask) {
            if (await validate(mission)) {
                await verifyTaskFields(mission);
                await verifyTaskContent(mission);
...
const verifyTaskFields = async(mission) => {
    return null;
};

const verifyTaskContent = async(mission) => {
    return null;
};
...
const toCallGrpc = async(currentNode) => {
    return new Promise((resolve, reject) => {
